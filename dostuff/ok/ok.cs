...
namespace Ok
{
    ...
    public class MultifilterTR : AsyncPositionedPipelineElement<IDeviceReport>
    {
        ...
        [BooleanProperty("Kalman Toggle", ""), DefaultPropertyValue(false), ToolTip
        (
            "false - 1\n" +
            "true - 2"
        )]
        public bool kToggle { set; get; }
        ...
        void StatUpdate(ITabletReport report)
        {
            ...
            if (frameShift > 0f) {
                ...
                if (tick > 90) {
                    Console.WriteLine(hcToggle && (emergency > 0));
                    sum1 += Vector2.DistanceSquared(smpos[0], h1);
                    sum2 += Vector2.DistanceSquared(smpos[0], h2);
                ...
                Vector2 c3 = ddir[0] - ddir[1];

                if ((kf != null) && (ts != null)) {
                    h1 = kf.Update(smpos[0], secAvg, hcToggle && (emergency > 0), c1, c2, c3);
                    h2 = ts.Update2(smpos[0], secAvg, hcToggle && (emergency > 0), c1, c2, c3);
                    if (kToggle)
                        predict = h2;
                    else
                        predict = h1;
        ...
        {
            kf = new KalmanVector2(2, p);
            ts = new KalmanVector2(2, p);
        ...
        public class KalmanFilter
        {
            ...
            public double Update(double measuredPos, double dt, bool nonconfident, float val1, float val2, float val3)
            {
            ...
            double vale = val2 + val3;

            double[,] qArr = new double[states, states];
            qArr[0, 0] = 0.1;
            qArr[1, 1] = 2.0;
            qArr[2, 2] = 5.0;
            qArr[3, 3] = 50.0;
            Matrix tQ = Matrix.Build.DenseOfArray(qArr);

            double[,] rArr = new double[3, 3];
            rArr[0, 0] = 0.00001;
            rArr[1, 1] = 0.5 - 0.49999 * Smoothstep(val1 + vale, 0.0, 10.0);
            rArr[2, 2] = 1.0 - 0.99999 * Smoothstep(val1 + vale, 5.0, 10.0);
            Matrix tR = Matrix.Build.DenseOfArray(rArr);
            ...
            public double Update2(double measuredPos, double dt, bool nonconfident, float val1, float val2, float val3)
            {
            double measuredVel = (measuredPos - lastMeasuredPos) / dt;
            double measuredAccel = (measuredVel - lastMeasuredVel) / dt;
            lastMeasuredPos = measuredPos;
            ...
            qArr[1, 1] = 2.0;
            qArr[2, 2] = 5.0;
            qArr[3, 3] = 50.0 * (1 + 100 * Smoothstep(vale + val3, 35.0, 752.0));
            Matrix tQ = Matrix.Build.DenseOfArray(qArr);
        ...
        public class KalmanVector2
        {
            ...
            public Vector2 Update(Vector2 measuredPosition, float dt, bool nonconfident, Vector2 val1, Vector2 val2, Vector2 val3)
            {
                float xState = (float)xFilter.Update(measuredPosition.X, dt, nonconfident, Math.Abs(val1.X), Math.Abs(val2.X), Math.Abs(val3.X));
                float yState = (float)yFilter.Update(measuredPosition.Y, dt, nonconfident, Math.Abs(val1.Y), Math.Abs(val2.Y), Math.Abs(val3.Y));
                return new Vector2(xState, yState);
            }

            public Vector2 Update2(Vector2 measuredPosition, float dt, bool nonconfident, Vector2 val1, Vector2 val2, Vector2 val3)
            {
                float xState = (float)xFilter.Update2(measuredPosition.X, dt, nonconfident, Math.Abs(val1.X), Math.Abs(val2.X), Math.Abs(val3.X));
                float yState = (float)yFilter.Update2(measuredPosition.Y, dt, nonconfident, Math.Abs(val1.Y), Math.Abs(val2.Y), Math.Abs(val3.Y));
                return new Vector2(xState, yState);
            }
        }
     ...
     /*   public class KalmanFilterq
        {
            ...
            public KalmanFilterq(uint statesNumber, double initialPosition)
            {
                states = (int)statesNumber + 2;
                ...
                double Base = 0.00001;
                double[,] rArr = new double[states, states];
                for (int i = 0; i < states; i++) {
                    rArr[i, i] = Base;
                    Base *= 10.0;
                }
                R = Matrix.Build.DenseDiagonal(3, 3, 0.0001);
                H2 = Matrix.Build.DenseDiagonal(3, states, 1);
            }
            ...
            public double Update2(double measuredPos, double dt, bool nonconfident)
            {
                double measuredVel = (measuredPos - lastMeasuredPos) / dt;
                double measuredAccel = (measuredVel - lastMeasuredVel) / dt;
                lastMeasuredPos = measuredPos;
                ...
                var S = H2 * P * H2.Transpose() + R * 1.0;
                var K = P * H2.Transpose() * S.Inverse();

                x = x + K * (z - H2 * x);
                P = (Matrix.Build.DenseIdentity(states) - K * H2) * P;

                return (A * x)[0, 0];
            }
        }

        public class KalmanVector2q
        {
            private KalmanFilterq xFilter;
            private KalmanFilterq yFilter;

            public KalmanVector2q(uint states, Vector2 initialPosition)
            {
                xFilter = new KalmanFilterq(states, initialPosition.X);
                yFilter = new KalmanFilterq(states, initialPosition.Y);
            }
            ...
            public Vector2 Update2(Vector2 measuredPosition, float dt, bool nonconfident)
            {
                float xState = (float)xFilter.Update2(measuredPosition.X, dt, nonconfident);
                float yState = (float)yFilter.Update2(measuredPosition.Y, dt, nonconfident);
                return new Vector2(xState, yState);
            }
        }*/
    } 
