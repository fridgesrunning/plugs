...
namespace Saturn
{
    [PluginName("Saturn - Multifilter (kalman velocity) WHY ARE YOU SNOOPING?")]
    public class MultifilterKV : AsyncPositionedPipelineElement<IDeviceReport>
    {
        public MultifilterKV() : base()
        {
        }
        ...
        [Property("Expected Milliseconds Per Report Override"), DefaultPropertyValue(3.302466f), ToolTip
        (
        ...
        [Property("Area Scale"), DefaultPropertyValue(1.0f), ToolTip
        (
        ...
        protected override void UpdateState()
        {
            ...
            {    
                ...
                if (emergency > 0) {
                    ...
                    InsertAtFirst(prdir, Vector2.Zero);
                ...
                useDir = RTrajectory(t, stdir[2], stdir[1], stdir[0]) / expectC;

                startOutput += WireMultAdjust(useDir, expect, updateTime, wire);

                Vector2 lol = smpos[0] + t * useDir;

                startOutput = Vector2.Lerp(startOutput, lol, Math.Min(1, WireMultAdjust(adjdWeight, expect, updateTime, wire)));
        ...
        void Plot() {
            Console.Write("vx");
            Console.WriteLine(((stdir[0]).X));
            Console.Write("vy");
            Console.WriteLine(((stdir[0]).Y) * -1);
            /*Console.Write("jx");
        ...
        void StatUpdate(ITabletReport report) {
            ...
            Vector2 predict = dir[0];
            float zeroscale = Smoothstep(dir[0].Length(), 5, 5);
            if (frameShift > 0f) {

                if (kf != null) predict = kf.Update(dir[0], secAvg);
                else predict = dir[0];

                predict += (dir[0] - predict) * (1f - (frameShift * (1f - zeroscale)));
            }
        ...
        void ResetValues(Vector2 p) {
            kf = new KalmanVector2(4, Vector2.Zero);
            stdir = Enumerable.Repeat(Vector2.Zero, stdir.Length).ToArray();
            smpos = Enumerable.Repeat(p, smpos.Length).ToArray();
            prdir = Enumerable.Repeat(Vector2.Zero, prdir.Length).ToArray();
